      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Form(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Définissez un code PIN à 4 chiffres pour sécuriser l\'accès.'),
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Form(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Veuillez saisir votre code PIN actuel pour désactiver le verrouillage.'),
              const SizedBox(height: 16),
              TextFormField(
                controller: pinController,
                obscureText: true,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                final success = await securityProvider.disableAppLock(pinController.text);
                if (success) {
                  if (ctx.mounted) Navigator.pop(ctx);
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
            ...
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                ...
                if (success) {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('✅ Code PIN modifié avec succès')),
                    );
                  }
                } else {
                  ScaffoldMessenger.of(ctx).showSnackBar(
                                ...
                                children: [
                                  if (authProvider.isAuthenticated) ...[
                                    ...
                                    ListTile(
                                      ...
                                      title: const Text('Mon code unique'),
                                      subtitle: Text(
                                        authProvider.user?['user_code'] ?? 'Génération...',
                                        style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                                      ),
                                      trailing: IconButton(
                                        icon: const Icon(Icons.copy_rounded),
                                        onPressed: () {
                                          final code = authProvider.user?['user_code'];
                                          if (code != null) {
                                            Clipboard.setData(ClipboardData(text: code));
                                            ScaffoldMessenger.of(context).showSnackBar(
                                              const SnackBar(content: Text('Code unique copié !')),
                                            );
                                          }
                                        },
                                      ),
                                    ...
                                    const Divider(height: 1, indent: 16),
                                    ListTile(
                                      leading: Icon(Icons.people_alt_rounded, color: theme.colorScheme.primary),
                                      title: const Text('Mes contacts'),
                                      subtitle: const Text('Gérer vos contacts pour le partage de comptes et dettes'),
                                      onTap: () {
                                        Navigator.push(
                                          context,
                                    ...
                                    ListTile(
                                      ...
                                      subtitle: const Text('Pousse vos données locales vers le cloud'),
                                      onTap: authProvider.isSyncing
                                          ? null
                                          : () async {
                                              final result = await authProvider.syncNow(context);
                                              if (context.mounted) {
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  SnackBar(
                          ...
                          _buildSectionCard(
                            ...
                            children: operators.isEmpty
                                ? [
                                    Padding(
                                      padding: const EdgeInsets.all(16.0),
                                      child: Text(l10n.noOperatorAvailable, style: const TextStyle(fontStyle: FontStyle.italic)),
                                    )
                                  ]
                                : operators.map((op) => Column(
                                      children: [
                                        ListTile(
                                          leading: CircleAvatar(
                                            backgroundColor: theme.colorScheme.primaryContainer,
                                            child: Icon(Icons.cell_tower, color: theme.colorScheme.onPrimaryContainer),
                                          ),
                                          title: Text(op.name, style: const TextStyle(fontWeight: FontWeight.w600)),
                                          subtitle: Text(op.userPhoneNumber ?? l10n.noNumberDefined),
                                          trailing: const Icon(Icons.chevron_right, color: Colors.grey),
                                          onTap: () {
                                            Navigator.push(
                                              context,
                                              MaterialPageRoute(
                                                builder: (_) => AddOperatorScreen(operator: op),
                                              ),
                                            );
                                          },
                                        ),
                                        if (op != operators.last) const Divider(height: 1, indent: 70),
                                      ],
                                    )).toList(),
                          ),
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        Text(l10n.appLanguage, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                      ],
                                    ),
                                    const SizedBox(height: 8),
                                        ...
                                        return Wrap(
                                          ...
                                          children: [
                                            ChoiceChip(
                                              ...
                                              onSelected: (selected) {
                                                if (selected) localeProvider.setLocale(const Locale('fr'));
                                              },
                                            ),
                                            ChoiceChip(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 8),
                                        Text(l10n.country, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                      ],
                                    ),
                                    const SizedBox(height: 8),
                                    Consumer<ProfileProvider>(
                                      builder: (context, profileProvider, child) {
                                        ...
                                        return SearchableCountryDropdown<String>(
                                          ...
                                          onChanged: (val) {
                                            profileProvider.updateProfile(
                                              profileProvider.profile.copyWith(country: val ?? 'Tous'),
                                            );
                                          },
                                        );
                                      },
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 8),
                                        Text(l10n.currency, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                      ],
                                    ),
                                    const SizedBox(height: 8),
                                    Consumer<ProfileProvider>(
                                      builder: (context, profileProvider, child) {
                                        ...
                                        final currencies = ['CFA', '€', '\$', '£', 'XOF', 'GNF', 'CDF'];
                                        return Wrap(
                                          spacing: 8.0,
                                          runSpacing: 8.0,
                                          children: currencies.map((c) {
                                            return ChoiceChip(
                                              ...
                                              onSelected: (selected) {
                                                if (selected) {
                                                  profileProvider.updateProfile(
                                                    profileProvider.profile.copyWith(currency: c),
                                                  );
                                                }
                                              },
                                            );
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 8),
                                        Text(l10n.themeColor, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                      ],
                                    ),
                                    const SizedBox(height: 8),
                                      ...
                                      builder: (context, themeProvider, child) {
                                        ...
                                        return Wrap(
                                          ...
                                          children: colors.map((c) {
                                            final isSelected = themeProvider.primaryColor.toARGB32() == c.toARGB32();
                                ...
                                children: [
                                  ListTile(
                                    leading: Icon(
                                      ...
                                      color: securityProvider.isAppLockEnabled ? theme.colorScheme.primary : Colors.grey,
                                    ),
                                    title: const Text('Verrouiller l\'application'),
                                    subtitle: const Text('Sécuriser l\'accès avec un code PIN'),
                                    trailing: Switch(
                                  ...
                                  if (securityProvider.isAppLockEnabled) ...[
                                    ...
                                    ListTile(
                                      ...
                                      title: const Text('Modifier le code PIN'),
                                      subtitle: const Text('Changer votre code PIN à 4 chiffres'),
                                    ...
                                    if (securityProvider.isBiometricsAvailable) ...[
                                      ...
                                      ListTile(
                                        leading: Icon(
                                          ...
                                          color: securityProvider.isBiometricEnabled ? theme.colorScheme.primary : Colors.grey,
                                        ),
                                        title: const Text('Empreinte / Face ID'),
                                        subtitle: const Text('Déverrouiller avec la biométrie'),
                                        trailing: Switch(
